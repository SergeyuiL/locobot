...
PlaneDetection::PlaneDetection(ros::NodeHandle nh)
{
	...
	seg_result_pub = it.advertise("plane_detection_result", 1);
	rgb_sub.subscribe(nh, "/locobot/camera/color/image_raw", 1);
    dep_align_sub.subscribe(nh, "/locobot/camera/aligned_depth_to_color/image_raw", 1);
	sync.reset(new message_filters::Synchronizer<RealScenseSyncPolicy>(RealScenseSyncPolicy(10), rgb_sub, dep_align_sub));
    sync->registerCallback(boost::bind(&PlaneDetection::realsenceCallback,this, _1, _2));
...
void PlaneDetection::realsenceCallback(const ImageConstPtr& imageRGB, const ImageConstPtr& imageDepAlgin)
{
	// ROS_INFO("Received rgb and depth image!");
	double start_time = ros::Time::now().toSec();
	cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(imageRGB, sensor_msgs::image_encodings::TYPE_8UC3);
	...
	cv_bridge::CvImagePtr depth_cv_ptr = cv_bridge::toCvCopy(imageDepAlgin, sensor_msgs::image_encodings::TYPE_16UC1);

	cv::Mat depth_img = depth_cv_ptr->image;
	...
	// ROS_INFO("plane detection time: %0.3f", end_time-start_time);
	sensor_msgs::ImagePtr rgb_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", seg_img_).toImageMsg();
	rgb_msg->header.stamp = imageRGB->header.stamp;
	rgb_msg->header.frame_id = imageRGB->header.frame_id;
	seg_result_pub.publish(rgb_msg);
	reset();
}
